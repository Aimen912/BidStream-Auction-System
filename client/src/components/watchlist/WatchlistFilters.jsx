...
} from '../../data/watchlist/WATCHLIST_DATA';
...
function SortMenu({ value, onChange }) {
  ...
  const activeLabel = WATCHLIST_SORT_OPTIONS.find((o) => o.value === value)?.label ?? 'Sort';
    ...
    <div ref={ref} className="relative shrink-0">
      ...
      >
        ...
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"
          className={['transition-transform duration-150', open ? 'rotate-180' : ''].join(' ')} aria-hidden="true">
          <polyline points="6 9 12 15 18 9" />
        </svg>
      </button>

      {open && (
        <ul role="listbox" className="absolute right-0 top-full z-30 mt-1.5 w-48 overflow-hidden rounded-xl border border-border bg-bg-card py-1 shadow-dropdown">
          {WATCHLIST_SORT_OPTIONS.map(({ value: v, label }) => {
            const isSel = value === v;
            return (
              <li
                key={v}
 ...
 */
function WatchlistFilters({ filters, onChange, onClear, hasActive }) {
  const STATUS_ACTIVE = {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-border bg-bg-card p-4 shadow-card lg:p-5">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-0.5" role="group" aria-label="Filter by status">
        {WATCHLIST_STATUSES.map(({ value, label }) => {
