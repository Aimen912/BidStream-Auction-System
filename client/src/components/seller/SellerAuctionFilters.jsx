...
import { MA_CATEGORIES, MA_STATUSES, MA_SORT_OPTIONS } from '../../data/seller/SELLER_AUCTIONS_DATA';
...
function SortMenu({ value, onChange }) {
  ...
  const label = MA_SORT_OPTIONS.find((o) => o.value === value)?.label ?? 'Sort';
    ...
    <div ref={ref} className="relative shrink-0">
      ...
      {open && (
        <ul role="listbox" className="absolute right-0 top-full z-30 mt-1.5 w-44 overflow-hidden rounded-xl border border-border bg-bg-card py-1 shadow-dropdown">
          {MA_SORT_OPTIONS.map(({ value: v, label: l }) => {
            const isSel = value === v;
            return (
 ...
 */
function SellerAuctionFilters({ filters, onChange, onClear, hasActive }) {
  const STATUS_ACTIVE = {
...
export default SellerAuctionFilters;
