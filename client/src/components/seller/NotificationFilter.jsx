...
import {
  ...
  STATUS_OPTIONS,
  SORT_OPTIONS,
} from '../../data/seller/SELLER_NOTIFICATIONS_DATA';
...
/**
 * Filter bar for the Seller Notifications page.
 *
 * @param {object}   filters   – { search, status, type, priority, sort }
 * @param {function} onChange  – (key, value) => void
 * @param {function} onClear
 * @param {boolean}  hasActive
 */
function NotificationFilter({ filters, onChange, onClear, hasActive }) {
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-border bg-bg-card p-4 shadow-card lg:p-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <SelectFilter value={filters.status}   onChange={(v) => onChange('status', v)}   options={STATUS_OPTIONS}   />
          <SelectFilter value={filters.priority} onChange={(v) => onChange('priority', v)} options={PRIORITY_OPTIONS} />
          <SelectFilter value={filters.sort}     onChange={(v) => onChange('sort', v)}     options={SORT_OPTIONS}     />
      ...
      {/* Type pills */}
      <div className="flex items-center gap-2 overflow-x-auto pb-0.5" role="group" aria-label="Filter by notification type">
        {NOTIFICATION_TYPES.map(({ value, label }) => {
          const isActive = filters.type === value;
          return (
            <button key={value} type="button" onClick={() => onChange('type', value)} aria-pressed={isActive}
              className={[
